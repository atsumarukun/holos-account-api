...
func TestAccount_SetName(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "full-width charactors", inputName: "アカウント名", expectError: status.ErrBadRequest},
...
func TestAccount_SetPassword(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "full-width charactors", inputPassword: "認証パスワードぱすわーど", expectError: status.ErrBadRequest},
