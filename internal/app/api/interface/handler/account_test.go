...
func TestAccount_UpdateName(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "success",
			requestJSON:    []byte(`{"name": "update"}`),
			isSetAccountID: true,
			expectCode:     http.StatusCreated,
			expectResponse: map[string]any{"name": "update"},
			setMockAccountUC: func(ctx context.Context, accountUC *usecase.MockAccountUsecase) {
				accountUC.
					EXPECT().
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c, _ := gin.CreateTestContext(w)
			var err error
			c.Request, err = http.NewRequestWithContext(ctx, "PUT", "/accounts/:id", bytes.NewBuffer(tt.requestJSON))
			if err != nil {
				t.Error(err)
			}
